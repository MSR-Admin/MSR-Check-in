...
(function () {
  ...
  const translations = {
    en: {
      ...
      // Updated to sentence case for a professional look per UI guide
      ...
      // Employee screen labels also updated to sentence case
      nameLabel: 'Full Name',
  ...
  const DEPARTMENTS = [
    ...
    { name: 'Business Development', persons: ['Cecil Dela Rosa'] },
    { name: 'Office of the President', persons: ['Charidy Ma-ao'] }
  ];
  ...
  function setFieldError(input, message) {
    var container = input.closest('.field-group') || input.closest('.emp-field') || input.parentElement;
    var errorEl = container ? container.querySelector('.error-message') : null;
    if (message) {
  ...
  form.addEventListener('submit', function (e) {
    ...
    var visitorData = { action: 'checkin', fullName: fullName.value.trim(), contactNumber: contactNumber.value.trim(), contactPerson: contactPerson.value, purpose: purpose.value };
      ...
      submitCheckin(visitorData).catch(function () {
  ...
  // Debug helper to verify modals exist
  console.log('Visitor modal exists:', !!document.getElementById('visitorSuccessModal'));
  console.log('Employee modal exists:', !!document.getElementById('employeeSuccessModal'));
  console.log('Toast exists:', !!document.getElementById('toast'));
  ...
  if (visitorBackArrow) {
    visitorBackArrow.addEventListener('click', function () {
      visitorBackArrow.classList.add('clicked');
      setTimeout(function () { visitorBackArrow.classList.remove('clicked'); }, 300);
      hideAllModals();
  ...
  if (employeeBackArrow) {
    employeeBackArrow.addEventListener('click', function () {
      employeeBackArrow.classList.add('clicked');
      setTimeout(function () { employeeBackArrow.classList.remove('clicked'); }, 300);
      hideAllModals();
      showScreen(welcomeScreen);
    });
  }

  // ─── Employee Login Tracking (limit temporarily removed) ──────
  ...
  // ─── Employee Check-in ─────────────────
  function submitEmployeeCheckin() {
  ...
  function updateClock() {
    ...
    // Use 12‑hour format with AM/PM for better readability on the dashboard
    var timeStr = now.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit', hour12: true });
